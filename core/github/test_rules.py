...
def test_sync_issues_keeps_unlabeled_open_when_filtering(gh, conn, admin, org, monkeypatch):
    """라벨로 걸러 읽었으면 목록에 없는 이슈는 닫힌 게 아니라 라벨이 없을 뿐이다."""
    RepoIssue.objects.create(connection=conn, number=2, title="라벨 없음", state="open")
    _fake_issue_api(
        monkeypatch, org, admin, [{"number": 1, "title": "이슈", "labels": [], "assignee": None}]
    )
    ...
    conn.save(update_fields=["import_label"])
    ghs.sync_issues(conn)
    assert RepoIssue.objects.get(connection=conn, number=2).state == "open"
