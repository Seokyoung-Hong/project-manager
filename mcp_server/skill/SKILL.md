...
거버넌스 9절의 절차를 도구로 옮긴 것이다. 브랜치·PR·이슈 연결 상태는 `get_task` 또는
`get_task_github`로 조회한다. 이슈 본문은 GitHub 웹훅/동기화가 저장한 캐시이며, 최신 시각을 확인한다.
저장소 접근 권한이 없으면 GitHub 자료는 반환되지 않는다. 브랜치·PR 생성 자체는 GitHub 웹 UI에서 한다.
태스크 상태는 GitHub 웹훅이 옮기므로 `transition_task`를 손으로 부르지 않는다.
